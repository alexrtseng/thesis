...
def pjm_deterministic_reg_arbitrage_battery_opt(
    reg_prices: pd.DataFrame,
    arbitrage_prices: pd.DataFrame,
    battery: BatteryParams = DEFAULT_BATTERY,
    verbose: bool = False,
    curr_reg_allocation: float = 0.0,
) -> tuple[pd.DataFrame, float]:
    """Specific to PJM market mechanisms."""
    ...
    for t in range(T - 1):
        ...
        # Only create reg_allocation variable at PJM reg bidding times
        if times[t].minute in (30, 0):
            r_allocation = model.addVar(lb=0.0, name=f"reg_allocation_{t}")
        else:
            try:
                r_allocation = reg_allocation[t - 1]
            except IndexError:
                r_allocation = model.addVar(
                    lb=curr_reg_allocation,
                    ub=curr_reg_allocation,
                    name=f"reg_allocation_{t}",
                )
...
if __name__ == "__main__":
    ...
    # Use 5-minute intervals so :25 and :55 appear in the demo sequence
    idx = pd.date_range("2025-01-05 00:00", periods=12 * 12, freq="5T", tz="UTC")
    ...
    res, total_profit = pjm_deterministic_reg_arbitrage_battery_opt(
